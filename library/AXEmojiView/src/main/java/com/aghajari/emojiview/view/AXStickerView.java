...
public class AXStickerView extends AXEmojiLayout {
    ...
    private void init() {
        ...
        vp.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int i) {
                ...
                if (!AXEmojiManager.getStickerViewTheme().isAlwaysShowDividerEnabled()) {
                    if (((AXStickerViewPagerAdapter) Objects.requireNonNull(vp.getAdapter())).recyclerViews.size() > i) {
    ...
    public void setPageIndex(int index) {
        ...
        if (!AXEmojiManager.getStickerViewTheme().isAlwaysShowDividerEnabled()) {
            if (((AXStickerViewPagerAdapter) Objects.requireNonNull(vp.getAdapter())).recyclerViews.size() > index) {
    ...
    protected void addItemDecoration(RecyclerView.ItemDecoration itemDecoration) {
        ((AXStickerViewPagerAdapter) Objects.requireNonNull(vp.getAdapter())).itemDecoration = itemDecoration;
    ...
    public void refreshNow(int scrollToPosition) {
        super.refresh();
        try {
            for (int i = 0; i < ((AXStickerViewPagerAdapter) Objects.requireNonNull(vp.getAdapter())).recyclerViews.size(); i++) {
                View view = ((AXStickerViewPagerAdapter) vp.getAdapter()).recyclerViews.get(i);
                if (view instanceof RecyclerView)
                    ((RecyclerView) view).getAdapter().notifyDataSetChanged();
            }
